#!/bin/bash
# Submit training jobs for Part 2 datasets with full dependency chaining
# Usage: ./submit_training_part2.sh <dataset_name>
#   or: ./submit_training_part2.sh all  (to submit all Part 2 datasets sequentially)
#
# Part 2 datasets: ATL5, ATL6, ATL7 (9 jobs total)
#
...
if [ -z "$DATASET_ARG" ]; then
    ...
    echo ""
    echo "Available datasets (Part 2):"
    for ds in "${!DATASET_TASKS[@]}"; do
        echo "  - $ds"
    done | sort
    echo "  - all (submit all Part 2 datasets sequentially)"
    exit 1
fi
...
if [ "$DATASET_ARG" == "all" ]; then
    echo "========================================="
    echo "Submitting Part 2 datasets SEQUENTIALLY"
    echo "========================================="
    echo ""
    echo "Total: 9 jobs (3 datasets × 3 seeds)"
    echo "Estimated time: ~9 hours total"
    echo ""
    echo "Chain: ATL5 s0→s1→s2 → ATL6 s0→s1→s2 → ATL7 s0→s1→s2"
    echo ""

    read -p "Submit all Part 2 jobs? (y/n) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Cancelled."
        exit 0
